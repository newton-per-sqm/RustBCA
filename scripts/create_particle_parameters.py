...
class Particles:
    ...
    def write_to_file(self, dump_to_file=False):
        ...
        def have_ending_zeros(lis):
            ...
            for i in range(len(lis)):
                if (
                    type(lis[i]) == np.float64
                    or type(lis[i]) == np.float32
                    or type(lis[i]) == int
                    or type(lis[i]) == float
                ):
                    lis[i] = decimal.Decimal(
                        ("{0:0." + str(len(str(lis[i]))) + "f}").format(lis[i]) + "0"
                    ...
                    for j in range(len(lis[i])):
                        if (
                            type(lis[i][j]) == np.float64
                            or type(lis[i][j]) == np.float32
                            or type(lis[i][j]) == int
                            or type(lis[i][j]) == float
                            or True
                        ):
                            lis[i][j] = decimal.Decimal(
                                ("{0:0." + str(len(str(lis[i][j]))) + "f}").format(
                                    lis[i][j]
                                )
                                + "0"
                            ...
                            for k in range(len(lis[i][j])):
                                if (
                                    ...
                                    or type(lis[i][j][k]) == np.float32
                                    or type(lis[i][j][k]) == int
                                    or type(lis[i][j][k]) == float
                                ):
                                    lis[i][j][k] = decimal.Decimal(
                                        (
                                            "{0:0." + str(len(str(lis[i][j][k]))) + "f}"
                                        ).format(lis[i][j][k])
                                        + "0"
                                    )  # .quantize(Decimal("1." + "0"*(int(number_of_decimals))))
...
if __name__ == "__main__":
    ...
    for i in range(100):
        particle.add_particle_species(
            ...
            0.5,
            10.0,
            position=[0.0, random.randint(0, 50.0), 0.0],
        )
