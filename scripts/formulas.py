...
def thomas_reflection(ion, target, energy_eV):
    ...
    reduced_energy = (
        32.55 * energy_keV * M2 / ((M1 + M2) * Z1 * Z2 * (Z1**0.23 + Z2**0.23))
    )
    ...
    return (
        a1(mu)
        * np.log(a2(mu) * reduced_energy + 2.718)
        / (1.0 + a3(mu) * reduced_energy ** a4(mu) + a5(mu) * reduced_energy ** a6(mu))
    )
...
def wierzbicki_biersack(ion, target, energy_eV):
    ...
    RN_mu = np.exp(a1 * np.sqrt(1.0 - b1 * (np.log(mu / c1)) ** 2.0))
    RN_e = a2 * np.exp(b2 * (np.log(reduced_energy + 1.0)) ** c2)
...
def bohdansky_heavy_ion(ion, target, energy_eV):
    ...
    a0 = 0.529 * ANGSTROM
    a = 0.885 * a0 * (z1 ** (2.0 / 3.0) + z2 ** (2.0 / 3.0)) ** (-1.0 / 2.0)
    reduced_energy = (
        0.03255
        / (z1 * z2 * (z1 ** (2.0 / 3.0) + z2 ** (2.0 / 3.0)) ** (1.0 / 2.0))
        * reduced_mass_2
        * energy_eV
    )
    sn = (
        3.441
        * np.sqrt(reduced_energy)
        * np.log(reduced_energy + 2.718)
        / (
            1.0
            + 6.355 * np.sqrt(reduced_energy)
            + reduced_energy * (-1.708 + 6.882 * np.sqrt(reduced_energy))
        )
    )
    Sn = (
        8.478
        * z1
        * z2
        / (z1 ** (2.0 / 3.0) + z2 ** (2.0 / 3.0)) ** (1.0 / 2.0)
        * reduced_mass_1
        * sn
    )

    sputtering_yield = (
        alpha * Sn * (1 - (Eth / energy_eV) ** (2.0 / 3.0)) * (1.0 - Eth / energy_eV) ** 2
    )
...
def yamamura(ion, target, energy_eV):
    ...
    k = (
        0.079
        * (m1 + m2) ** (3.0 / 2.0)
        / (m1 ** (3.0 / 2.0) * m2 ** (1.0 / 2.0))
        * z1 ** (2.0 / 3.0)
        * z2 ** (1.0 / 2.0)
        / (z1 ** (2.0 / 3.0) + z2 ** (2.0 / 3.0)) ** (3.0 / 4.0)
    )
