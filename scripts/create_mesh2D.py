...
np.set_printoptions(
    formatter={"float": lambda x: "{0:0." + number_of_decimals + "f}".format(x) + "0"}
)
...
class Mesh:
    ...
    def __init__(
        ...
        ymax,
        ymin,
        energy_barrier_thickness=1.7645653881793786e-4,
    ):
        """
        returns none.
    ...
    def N_gon(
        ...
        for i in range(n_points):
            temp_points.append(
                Point(
                    x_offset + radius * math.cos(dtheta * i + theta_offset),
                    y_offset + radius * math.sin(dtheta * i + theta_offset),
                )
            )
    ...
    def triangle(
        self, arm1, arm2, theta, number_densities, x_offset=0, y_offset=0, theta_offset=0
    ):
        """
        returns True on completion
        ...
        """
        point1 = Point(
            x_offset + arm1 * math.cos(theta_offset),
            y_offset + arm1 * math.sin(theta_offset),
        )
        point2 = Point(
            x_offset + arm2 * math.cos(theta + theta_offset),
            y_offset + arm2 * math.sin(theta + theta_offset),
        )
    ...
    def rectangle(
        ...
        for i in range(0, 2):
            for j in range(0, 2):
                temp_points.append(
                    Point(
                        x_offset + (-1) ** i * length_x / 2.0,
                        y_offset + (-1) ** (i ^ j) * length_y / 2.0,
                    )
                )
        ...
        for j in range(n_y):
            for i in range(n_x):
                self.rectangle(
                    ...
                    length_y,
                    number_densities,
                    x_offset=(bottom_left_x + length_x / 2 + i * length_x),
                    y_offset=(bottom_left_y + length_y / 2 + j * length_y),
                )
    ...
    def return_Triangles(self):
        ...
        for i in range(len(tri.simplices)):
            point_output.append(
                [
                    points[tri.simplices[i, 0], 0],
                    points[tri.simplices[i, 1], 0],
                    points[tri.simplices[i, 2], 0],
                    points[tri.simplices[i, 0], 1],
                    points[tri.simplices[i, 1], 1],
                    points[tri.simplices[i, 2], 1],
                ]
            )
            triangles.append(
                Polygon(
                    [
                        points[tri.simplices[i, 0]],
        ...
        for i, triangle in enumerate(triangles):
            for shape, number_densities in self.shapes:
                if (
                    shape.contains(triangle)
                    or shape.relate_pattern(triangle, "FF2FF1212")
                    or shape.relate_pattern(triangle, "212101212")
                ):
    ...
    def print_Triangles(self):
        ...
        for t in triangle_list:
            triangles.append(matplotlib.tri.Triangulation(t[0:3], t[3:6]))
        ...
        for i, triangle in enumerate(triangles):
            if type(material_densities[i]) == list:
                plt.triplot(
                    triangle,
                    self.color_dict[material_densities[i][0]] + "-",
                )
            else:
                plt.triplot(
                    triangle,
                    self.color_dict[material_densities[i]] + "-",
                )
        plt.show()
        return True
    ...
    def write_to_file(self, dump_to_file=False):
        ...
        material_boundary_points = list(
            material_boundary_points
            for material_boundary_points, _ in itertools.groupby(material_boundary_points)
        )
