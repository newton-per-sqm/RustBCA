...
def do_trajectory_plot(
    name,
    thickness=None,
    depth=None,
    boundary=None,
    plot_final_positions=True,
    plot_origins=True,
    show=True,
):
    """
    ...
    trajectories = np.atleast_2d(
        np.genfromtxt(name + "trajectories.output", delimiter=",")
    )
    trajectory_data = np.atleast_1d(
        np.genfromtxt(name + "trajectory_data.output", delimiter=",")
        .transpose()
        .astype(int)
    )
    ...
    if np.size(trajectories) > 0:
        ...
        colormap = cm.ScalarMappable(
            norm=colors.SymLogNorm(min_Z * 4, vmin=min_Z, vmax=max_Z), cmap="tab20"
        )
        ...
        if plot_final_positions:
            ...
            if np.size(deposited) > 0:
                ...
                plt.scatter(
                    ...
                    deposited[:, 3],
                    s=50,
                    color=deposited_colors,
                    marker="^",
                )

        if thickness and depth:
            x_box = [0.0, 0.0, depth, depth, 0.0]
            y_box = [
                -thickness / 2.0,
                thickness / 2.0,
                thickness / 2.0,
...
def do_trajectory_plot_3d(
    ...
    if np.size(trajectories) > 0:
        for trajectory_length in trajectory_data:
            ...
            if plot_origins:
                points3d(x[0], y[0], z[0], color=colormap.to_rgba(Z)[:3], scale_factor=1)

            if len(x) > min_length:
                plot3d(x, y, z, color=colormap.to_rgba(Z)[:3], tube_radius=0.5)
        ...
        if plot_final_positions:
            if np.size(sputtered) > 0:
                ...
                for x, y, z, c in zip(
                    sputtered[:, 3], sputtered[:, 4], sputtered[:, 5], sputtered_colors
                ):
                    points3d(
                        x * scale_factor,
                        y * scale_factor,
            ...
            if np.size(reflected) > 0:
                ...
                for x, y, z, c in zip(
                    reflected[:, 3], reflected[:, 4], reflected[:, 5], reflected_colors
                ):
                    points3d(
                        x * scale_factor,
                        y * scale_factor,
            ...
            if np.size(deposited) > 0:
                ...
                for x, y, z, c in zip(
                    deposited[:, 2], deposited[:, 3], deposited[:, 4], deposited_colors
                ):
                    points3d(
                        x * scale_factor,
                        y * scale_factor,
        ...
        if cube_length:
            ...
            xmin = -cube_length / 2.0 * scale_factor
            xmax = cube_length / 2.0 * scale_factor
            ymin = -cube_length / 2.0 * scale_factor
            ymax = cube_length / 2.0 * scale_factor
            zmin = -cube_length / 2.0 * scale_factor
            zmax = cube_length / 2.0 * scale_factor
...
def generate_rustbca_input(
    ...
    high_energy=False,
    energy_barrier_thickness=(6.306e10) ** (-1 / 3.0),
    initial_particle_position=-1 * ANGSTROM / MICRON,
    integral="MENDENHALL_WELLER",
    root_finder="DEFAULTNEWTON",
    delta_x_angstrom=5.0,
    ...
    minx, miny, maxx, maxy = 0.0, -thickness / 2.0, depth, thickness / 2.0
    ...
    geometry_input = {
        "length_unit": "MICRON",
        "triangles": [
            [0.0, depth, 0.0, thickness / 2.0, -thickness / 2.0, -thickness / 2.0],
            [0.0, depth, depth, thickness / 2.0, thickness / 2.0, -thickness / 2.0],
        ],
    ...
    cosx = np.cos(theta * np.pi / 180.0)
    sinx = np.sin(theta * np.pi / 180.0)
    if uniformly_distributed_ions:
        positions = [
            (
                initial_particle_position,
                np.random.uniform(-thickness / 2.0, thickness / 2.0),
                0.0,
            )
            for _ in range(N)
        ]
    else:
        positions = [(initial_particle_position, 0.0, 0.0) for _ in range(N)]
    ...
    particle_parameters = {
        ...
        "pos": positions,
        "dir": [(cosx, sinx, 0.0) for _ in range(N)],
...
def plot_distributions_rustbca(
    name,
    beam,
    target,
    incident_energy=1,
    incident_angle=0,
    max_collision_contours=4,
    plot_2d_reflected_contours=False,
    collision_contour_significance_threshold=0.1,
    ...
    if np.size(r) > 0:
        ...
        uy = r[:, 7]
        uz = r[:, 8]
        theta = -np.arctan2(ux, np.sqrt(uy**2 + uz**2)) * 180.0 / np.pi
        theta = 180.0 - np.arccos(ux) * 180.0 / np.pi
        ...
        c2 = ax.scatter(
            np.arccos(r[:, 7]), normalized_energies_rust, s=1, c=number_collision_events
        )
        ...
        plt.title(
            f"{beam['symbol']} on {target['symbol']} E0 = {np.round(incident_energy, 1)} eV {np.round(incident_angle, 1)} deg",
            fontsize="small",
        )
        ...
        bin_energy = np.linspace(0.0, 1.2 * np.max(r[:, 2]), num_bins)
        bin_angle = np.linspace(0.0, 90.0, num_bins)
        ...
        if plot_2d_reflected_contours:
            ...
            for k in range(1, n_max + 1):
                ...
                heights, xedges, yedges = np.histogram2d(
                    r[mask, 2], theta[mask], density=True, bins=num_bins // 2
                )
                ...
                plt.contour(
                    x_centers,
                    y_centers,
                    heights.transpose() / np.max(heights),
                    levels=np.linspace(collision_contour_significance_threshold, 1.0, 1),
                    colors=[colors[k - 1]],
                    linewidths=1,
                    linestyles="--",
                    alpha=0.75,
            ...
            cbar = plt.colorbar(
                sm,
                ticks=np.array(range(1, n_max + 1)) - 0.5,
                boundaries=np.array(range(0, n_max + 1)),
            )
        ...
        if plot_scattering_energy_curve:
            for mass in [titanium["m"], oxygen["m"]]:
                energies = bins[1]
                angles = bins[0] * np.pi / 180.0
                scattering_angles = -angles + np.pi
                final_energies = (
                    incident_energy
                    * (
                        (
                            np.cos(scattering_angles)
                            + np.sqrt(
                                (mass / beam["m"]) ** 2 - np.sin(scattering_angles) ** 2
                            )
                        )
                        / (1.0 + mass / beam["m"])
                    )
        ...
        plt.title(
            f"Reflected EAD {beam['symbol']} on {target['symbol']}", fontsize="small"
        )
    ...
    if np.size(s) > 0:
        ...
        bin_energy = np.linspace(0.0, 1.2 * np.max(s[:, 2]), num_bins // 2)
        bin_angle = np.linspace(0.0, 90.0, num_bins // 2)
        bins = (bin_angle, bin_energy)
        ux = s[:, 6]
        uy = s[:, 7]
        uz = s[:, 8]
        ...
        plt.xlabel("angle [deg]")
        plt.title(
            f"Sputtered EAD {beam['symbol']} on {target['symbol']}", fontsize="small"
        )
    ...
    if np.size(d) > 0:
        ...
        plt.title(
            f"Depth Distributions {beam['symbol']} on {target['symbol']} E0 = {np.round(incident_energy, 1)} eV {np.round(incident_angle, 1)} deg",
            fontsize="small",
        )
        plt.xlabel("x [um]")
        plt.ylabel("f(x)")
    ...
    if np.size(r) > 0:
        ...
        plt.title(
            f"Refl. Energies {beam['symbol']} on {target['symbol']} E0 = {np.round(incident_energy, 1)} eV {np.round(incident_angle, 1)} deg",
            fontsize="small",
        )
        plt.xlabel("E/E0")
        plt.ylabel("f(E)")
    ...
    if np.size(s) > 0:
        ...
        _, _, rust = plt.hist(
            s[:, 2], histtype="step", bins=num_bins, density=True, color="black"
        )
        plots.append(rust[0])
        labels.append("rustbca")

        energies = np.linspace(0.0, np.max(s[:, 2]), num_bins * 10)
        ...
        plt.title(
            f"Sputtered Energies {beam['symbol']} on {target['symbol']} E0 = {np.round(incident_energy, 1)} eV {np.round(incident_angle, 1)} deg",
            fontsize="small",
        )
        plt.xlabel("E [eV]")
        plt.ylabel("f(E)")
    ...
    if ax:
        ...
        ax.set_xticklabels(["0", "30", "60", "90", "", "", ""])
    plt.title(
        f"Sputtered Angles {beam['symbol']} on {target['symbol']} E0 = {np.round(incident_energy, 1)} eV {np.round(incident_angle, 1)} deg",
        fontsize="small",
    )
    ...
    if ax:
        ...
        ax.set_xticklabels(["0", "30", "60", "90", "", "", ""])
    plt.title(
        f"Refl. Angles {beam['symbol']} on {target['symbol']} E0 = {np.round(incident_energy, 1)} eV {np.round(incident_angle, 1)} deg",
        fontsize="small",
    )
...
def plot_energy_loss(name, N, num_bins=50, thickness=None, depth=None):
    ...
    energy_loss = np.loadtxt(
        f"{name}energy_loss.output", delimiter=",", usecols=[2, 3, 4, 5], dtype=np.float32
    )
    ...
    if depth and thickness:
        num_bins = (
            np.linspace(0.0, depth, num_bins),
            np.linspace(-thickness / 2.0, thickness / 2.0, num_bins),
        )
    ...
    Hn, bins_x, bins_y, _ = plt.hist2d(
        x, y, weights=En / N, bins=num_bins, norm=colors.SymLogNorm(0.1)
    )
    ...
    # plt.axis('square')
    He, bins_x, bins_y, _ = plt.hist2d(
        x, y, weights=Ee / N, bins=num_bins, norm=colors.SymLogNorm(0.1)
    )
...
def plot_all_depth_distributions(name, displacement_energy, N, num_bins=100):
    ...
    plt.legend(
        [
            ...
            "ΔE Electronic",
            "ΔE Total",
            f"Damage Ed = {displacement_energy}",
            "Deposited",
        ]
    )
    plt.xlabel("x [um]")
...
def run_iead(ions, target, energies, angles, iead, name="default_", N=1):
    ...
    N_ = np.array(
        [iead[i, j] * N for i in range(len(energies)) for j in range(len(angles))],
        dtype=int,
    )
    ...
    dx = 5.0 * ANGSTROM / MICRON

    minx, miny, maxx, maxy = 0.0, -thickness / 2.0, depth, thickness / 2.0
    surface = box(minx, miny, maxx, maxy)

    simulation_surface = surface.buffer(10.0 * dx, cap_style=2, join_style=2)
    geometry_input = {
        "length_unit": "MICRON",
        "energy_barrier_thickness": sum(n) ** (-1.0 / 3.0) / np.sqrt(2.0 * np.pi),
        "triangles": [
            [0.0, depth, 0.0, thickness / 2.0, -thickness / 2.0, -thickness / 2.0],
            [0.0, depth, depth, thickness / 2.0, thickness / 2.0, -thickness / 2.0],
        ],
    ...
    cosx = np.cos(theta * np.pi / 180.0)
    sinx = np.sin(theta * np.pi / 180.0)
    positions = [(-dx, 0.0, 0.0) for _ in range(len(N_))]
    ...
    particle_parameters = {
        ...
        "pos": positions,
        "dir": [(cx, sx, 0.0) for cx, sx in zip(cosx, sinx)],
        "particle_input_filename": "",
    }
...
def beam_target(
    ...
    generate_rustbca_input(
        ...
        delta_x_angstrom=2.0 * pmax * MICRON / ANGSTROM,
        initial_particle_position=-2.01 * pmax,
        weak_collision_order=weak_collision_order,
        ck=ck,
        uniformly_distributed_ions=uniformly_distributed_ions,
        mean_free_path_model=mean_free_path_model,
    )
    ...
    if do_plots:
        if track_energy_losses:
            plot_energy_loss(name, N * N_, num_bins=100)

        if plot_depth_distributions and track_displacements and track_recoils:
            plot_all_depth_distributions(name, 38.0, N_ * N, num_bins=200)

        if plot_distributions:
            plot_distributions_rustbca(
                name, ions, target, incident_angle=angle, incident_energy=energy
            )

        if track_displacements and track_recoils:
            plot_displacements(name, 20.0, num_bins=200)

        if track_trajectories and plot_trajectories:
            do_trajectory_plot(
                name, thickness=thickness, depth=depth, plot_final_positions=True
            )
...
def sputtering(ions, target, energies, angle, N_=10000, run_sim=True):
    ...
    for option_index, option in enumerate(options):
        ...
        for index, energy in enumerate(energies):
            generate_rustbca_input(
                ...
                thickness,
                depth,
                name=name + str(index) + "_" + str(option_index),
                track_trajectories=track_trajectories,
                track_recoil_trajectories=track_trajectories,
                track_energy_losses=track_energy_losses,
                track_displacements=track_displacements,
                track_recoils=True,
                electronic_stopping_mode="LOW_ENERGY_EQUIPARTITION",
                integral=integral,
            ...
            if run_sim:
                os.system(
                    f"RustBCA.exe {name + str(index) + '_' + str(option_index)}.toml"
                )

            sputtered = np.atleast_2d(
                np.genfromtxt(
                    name + str(index) + "_" + str(option_index) + "sputtered.output",
                    delimiter=",",
                )
            )
            reflected = np.atleast_2d(
                np.genfromtxt(
                    name + str(index) + "_" + str(option_index) + "reflected.output",
                    delimiter=",",
                )
            )
            ...
            if track_energy_losses:
                plot_energy_loss(
                    name + str(index) + "_" + str(option_index), N * N_, num_bins=200
                )

            if plot_depth_distributions:
                plot_all_depth_distributions(
                    name + "_" + str(index) + str(option_index),
                    38.0,
                    N_ * N,
                    num_bins=200,
                )
            ...
            if plot_distributions:
                plot_distributions_rustbca(
                    name + str(index) + "_" + str(option_index),
                    hydrogen,
                    boron,
    ...
    for energy in energies:
        sputteredf, reflectedf, _ = ftridyn.run_tridyn_simulations_from_iead(
            [energy], [angle], [[N_]], number_histories=N
        )
...
def plot_3d_distributions(ions, target, name):
    ...
    num_bins = 25
    s_dist, _ = np.histogramdd(
        np.array([s_ux, s_uy, s_uz]).transpose(), bins=num_bins, normed=False
    )
    ...
    r_dist, _ = np.histogramdd(
        np.array([r_ux, r_uy, r_uz]).transpose(), bins=num_bins, normed=False
    )
...
def metal_oxide_bilayer_iead(
    ...
    # build geometry
    minx, miny, maxx, maxy = 0.0, -thickness / 2.0, total_depth, thickness / 2.0
    material_boundary = box(minx, miny, maxx, maxy)
    dx = 5 * ANGSTROM / MICRON
    simulation_boundary = material_boundary.buffer(10.0 * dx, cap_style=2, join_style=2)

    # x1, x2, x3, y1, y2, y3
    triangles = [
        [0.0, layer_depth, 0.0, -thickness / 2.0, thickness / 2.0, thickness / 2.0],
        [
            0.0,
            layer_depth,
            layer_depth,
    ...
    metal_number_density = (
        metal_oxide_number_density * metal_stoichiometry / total_stoichiometry
    )
    oxygen_number_density = (
        metal_oxide_number_density * oxygen_stoichiometry / total_stoichiometry
    )

    densities = [
        [metal_number_density, oxygen_number_density],
        [metal_number_density, oxygen_number_density],
        [metal["n"] * (MICRON**3), 0.0],
        [metal["n"] * (MICRON**3), 0.0],
    ]
    ...
    geometry_input = {
        "length_unit": "MICRON",
        "energy_barrier_thickness": sum(densities[0]) ** (-1.0 / 3.0)
        / np.sqrt(2.0 * np.pi),
        "triangles": triangles,
        "densities": densities,
    ...
    sinx = np.sin(theta * np.pi / 180.0)
    positions = [
        (-dx / 2.0, np.random.uniform(-spread / 2.0, spread / 2.0), 0.0)
        for _ in range(len(N_))
    ]
    ...
    if run_sim:
        os.system(f"cargo run --release {name}.toml")
...
def helium_on_tungsten_oxide_layer():
    ...
    for j, layer_depth in enumerate(layer_depths):
        ...
        for i, porosity in enumerate(porosities):
            metal_oxide_number_density = (1.0 - porosity) * 1.004e10

            N, s, r, d = metal_oxide_bilayer_iead(
                ions,
                iead,
                energies,
                angles,
                ...
                thickness,
                N=0.01,
                tag=f"{str(i)}_{str(j)}_",
                run_sim=True,
                spread=5 * ANGSTROM / MICRON,
            )

            # deposition profile
            plt.figure(j)
            heights, bins, patches = plt.hist(
                d[:, 2], bins=num_bins, histtype="step", linewidth=2
            )
            handles.append(patches[0])
            legends.append(
                f"Porosity: {porosity * 100}% R: {np.round(np.shape(r)[0] / N, 2)} Y: {np.round(np.shape(s)[0] / N, 2)} [at/ion]"
            )

        plt.text(
            layer_depth - (layer_depths[0]) / 8, 0.0, "WO2", horizontalalignment="right"
        )
        plt.text(
            layer_depth + (layer_depths[0]) / 8, 0.0, "W", horizontalalignment="left"
        )
        plt.plot(
            [layer_depth, layer_depth],
            [0.0, 2 * np.max(heights)],
            linewidth=3,
            color="dimgray",
        )
        ...
        plt.axis([0.0, 1.5 * np.max(d[:, 2]), 0.0, 1.2 * np.max(heights)])
        plt.savefig(f"dep_west_1_wo2_w_{j}.png")


def generate_rustbca_input_sphere(
    Zb,
    Mb,
    n,
    Eca,
