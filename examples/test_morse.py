...
def run_morse_potential(energy, index, num_samples=10000, run_sim=True):
    ...
    if run_sim:
        os.system(f"cargo run --release --features cpr_rootfinder 0D morse_{index}.toml")

    reflected_list = np.atleast_2d(
        np.genfromtxt(f"morse_{index}reflected.output", delimiter=",")
    )
...
def run_krc_morse_potential(energy, index, num_samples=10000, run_sim=True):
    ...
    if run_sim:
        os.system(
            f"cargo run --release --features cpr_rootfinder 0D krc_morse_{index}.toml"
        )

    reflected_list = np.atleast_2d(
        np.genfromtxt(f"krc_morse_{index}reflected.output", delimiter=",")
    )
    if np.size(reflected_list) > 0:
        num_reflected = np.shape(reflected_list)[0]
        energy_reflected = np.sum(reflected_list[:, 2])
...
for index, energy in enumerate(energies):
    R_N[index], R_E[index] = run_krc_morse_potential(
        energy, index, num_samples=num_samples, run_sim=True
    )
    R_N_2[index], R_E_2[index] = run_morse_potential(
        energy, index, num_samples=num_samples, run_sim=True
    )
...
r_rustbca = np.array(
    [reflection_coefficient(hydrogen, nickel, energy, 0.0, 10000) for energy in energies]
)
r_n = r_rustbca[:, 0]
r_e = r_rustbca[:, 1]
