...
def main():
    ...
    print(
        f"Sputtering yield for {ion['symbol']} on {target['symbol']} at {energy} eV is {Y} at/ion. Yamamura predicts {np.round(yamamura(ion, target, energy), 3)} at/ion."
    )

    R_N, R_E = reflection_coefficient(ion, target, energy, angle, num_samples)
    print(
        f"Particle reflection coefficient for {ion['symbol']} on {target['symbol']} at {energy} eV is {R_N}. Thomas predicts {np.round(thomas_reflection(ion, target, energy), 3)}."
    )
    print(
        f"Energy reflection coefficient for {ion['symbol']} on {target['symbol']} at {energy} eV is {R_E}"
    )

    R_N, R_E = compound_reflection_coefficient(
    ...
    ux = np.cos(angle * np.pi / 180.0) * np.ones(number_ions)
    uy = np.sin(angle * np.pi / 180.0) * np.ones(number_ions)
    uz = np.zeros(number_ions)

    print(
        f"Running RustBCA for {number_ions} {ion['symbol']} ions on {target['symbol']} at {energies_eV[0] / 1000.0} keV..."
    )
    ...
    print(
        f"Running RustBCA for {number_ions} {ion['symbol']} ions on {target['symbol']} with hydrogenated layer at {energies_eV[0] / 1000.0} keV..."
    )
    print(f"This may take several minutes.")
    ...
    output, incident, stopped = compound_bca_list_1D_py(
        ...
        [target["Es"], 1.5],
        [target["Eb"], 0.0],
        [[target["n"] / 10**30, target["n"] / 10**30], [target["n"] / 10**30, 0.0]],
        [50.0, 1e6],
    )
    ...
    plt.figure(3)
    plt.title(
        f"Implanted {ion['symbol']} Depth Distribution with 50A {target['symbol']}-H layer"
    )
    plt.xlabel("x [A]")
    plt.ylabel("f(x) [A.U.]")
    heights, _, _ = plt.hist(
        x[np.logical_and(incident, stopped)], bins=100, density=True, histtype="step"
    )
    ...
    print(
        f"Running tracked RustBCA for {number_ions} {ion['symbol']} ions on {target['symbol']} with 1% {ion['symbol']} at {energies_eV[0] / 1000.0} keV..."
    )
    print(f"This may take several minutes.")
    ...
    output, incident, incident_index = compound_bca_list_tracked_py(
        ...
        [target["Ec"], ion["Ec"]],
        [target["Es"], ion["Es"]],
        [target["n"] / 10**30, 0.01 * target["n"] / 10**30],
        [target["Eb"], 0.0],
    )
    stop = time.time()
    delta_time = stop - start
